...
use Modules\ProfilePage\App\Http\Interfaces\EditProfilePageInterface;

class EditProfileController extends Controller implements EditProfilePageInterface
{
    ...
    public function update(Request $request): JsonResponse
    {
        $data = array_filter($request->validate([
            'profileId' => ['integer', Rule::exists('users', 'id')],
            'email' => ['nullable', 'email', 'regex:/^[\w\.~-]+@([a-zA-Z-]+\.)+[a-zA-Z-]{2,4}$/i',
                Rule::unique('users', 'email')->ignore($request->input('profileId')),
                'string', 'max:255'],
            'password' => ['nullable', 'min:6', 'max:255'],
            'name' => ['nullable', 'string', 'max:255'],
            'cid' => ['nullable', Rule::unique('users', 'cid')->ignore($request->input('profileId')),
        ...
        $userOld = Users::find($data['profileId']);
